...
async def send_reminders_hourly(context: CallbackContext):
    now = datetime.now()
    link = (await ExternalLink.objects.aget(source=LinkSources.FORM_PERSONAL_LESSONS)).link
    unfinished_states = {
    ...
    users = User.objects.filter(
        last_visit_time__gte=(now - timedelta(days=1)),
